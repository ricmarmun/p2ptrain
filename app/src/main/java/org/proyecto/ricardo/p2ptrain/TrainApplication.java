 ...
 * user interface design pattern known as Model-View-FController) for the chat
 * application.
 *
 * The TrainApplication inherits from the relatively little-known Android
 ...
 * recreated.
 */
public class TrainApplication extends Application implements Observable {
    private static final String TAG = "chat.TrainApplication";
     ...
     * ic_stop -- as they should.  One of the things that will ic_stop is the AllJoyn
    ...
    /**
     * The object we chat in notifications to indicate that an AllJoyn error has
     * happened.
     */
    ...
    public synchronized void addFoundChannel(String channel) {
        ...
        notifyObservers(CHAT_CHANNEL_FOUND_EVENT);
    }

    /**
     * The object we chat in notifications to indicate that the state of the
     * "chat" channel or its name has changed.
     */
    public static final String CHAT_CHANNEL_FOUND_EVENT = "CHAT_CHANNEL_FOUND_EVENT";
    ...
    /**
     * Whenever the user is asked for a channel to join, it nees the custom_list of
     * channels found via FoundAdvertisedName.  This method provides that
     * custom_list.  Since we have no idea how or when the caller is going to access
     * or change the custom_list, and we are deeply paranoid, we provide a deep copy.
     */
    ...
    /**
     * The channels custom_list is the custom_list of all well-known names that correspond to
     * channels we might conceivably be interested in.  We expect that the
     * "chat" GUID will allow the local user to have this custom_list displayed in a
    ...
    /**
     * Get the state of the "chat" channel.
     */
    ...
    /**
     * Set the name part of the "host" channel.  Since we are going to "chat" a
     * channel that is implemented remotely and discovered through an AllJoyn
     * FoundAdvertisedName, this must come from a custom_list of advertised names.
     * These names are our channels, and so we expect the GUI to choose from
     * among the custom_list of channels it retrieves from getFoundChannels().
     *
    ...
    public synchronized void hostSetChannelExercise(String ej) {
        mHostChannelExercise = ej;
        notifyObservers(HOST_CHANNEL_STATE_CHANGED_EVENT);
    }

    public synchronized void hostSetChannelDescription(String name) {
        mHostChannelDescription = name;
        notifyObservers(HOST_CHANNEL_STATE_CHANGED_EVENT);
    }

    /**
     * Get the name part of the "chat" channel.
     */
    public synchronized String hostGetChannelName() {
        return mHostChannelName;
    }
    public synchronized String hostGetEj() {
        return mHostChannelExercise;
    }
    ...
    private String mHostChannelExercise;
    private String mHostChannelDescription;
    ...
    /**
     * Set the status of the "chat" channel.  The AllJoyn Service part of the
     * appliciation is expected to make this call to set the status to reflect
     * the status of the underlying AllJoyn session.
     */
    public synchronized void chatSetChannelState(AllJoynService.ChatChannelState state) {
        mChatChannelState = state;
        notifyObservers(CHAT_CHANNEL_STATE_CHANGED_EVENT);
    }

    /**
     * Get the state of the "chat" channel.
     */
    public synchronized AllJoynService.ChatChannelState chatGetChannelState() {
        return mChatChannelState;
    }

    /**
     * The "chat" state which reflects the state of the part of the system
     ...
     * this sample.
     */
    private AllJoynService.ChatChannelState mChatChannelState = AllJoynService.ChatChannelState.IDLE;

    /**
     * The name of the "chat" channel which the user has selected.
     */
    private String mUseChannelName;
    private String mUseChannelEj;

    /**
     * Set the name part of the "chat" channel.  Since we are going to "chat" a
     * channel that is implemented remotely and discovered through an AllJoyn
     * FoundAdvertisedName, this must come from a custom_list of advertised names.
     * These names are our channels, and so we expect the GUI to choose from
     * among the custom_list of channels it retrieves from getFoundChannels().
     ...
     * this point.
     */
    public synchronized void chatSetChannelName(String name) {
        mUseChannelName = name;
        notifyObservers(CHAT_CHANNEL_STATE_CHANGED_EVENT);
    }

    public synchronized void chatSetChannelEj(String name) {
        mUseChannelEj = name;
    }
     ...
     * Get the name part of the "chat" channel.
     */
    public synchronized String chatGetChannelName() {
        return mUseChannelName;
    }

    public synchronized String chatGetChannelEj() {
        return mUseChannelEj;
    }
     ...
     * "chat" channel or its name has changed.
     */
    public static final String CHAT_CHANNEL_STATE_CHANGED_EVENT = "CHAT_CHANNEL_STATE_CHANGED_EVENT";

    /**
     * This is the method that the "chat" tab user interface calls when the
     ...
     * The "chat" channel is the channel that we talk about in the "Use" tab.
     ...
     */
    public synchronized void chatJoinChannel() {
        clearHistory();
        notifyObservers(CHAT_CHANNEL_STATE_CHANGED_EVENT);
        notifyObservers(CHAT_JOIN_CHANNEL_EVENT);
    }

    /**
     * The object we chat in notifications to indicate that user has requested
     * that we join a channel in the "chat" tab.
     */
    public static final String CHAT_JOIN_CHANNEL_EVENT = "CHAT_JOIN_CHANNEL_EVENT";
     ...
     */
    public synchronized void chatLeaveChannel() {
        notifyObservers(CHAT_CHANNEL_STATE_CHANGED_EVENT);
        notifyObservers(CHAT_LEAVE_CHANNEL_EVENT);
    }

    /**
     * The object we chat in notifications to indicate that user has requested
     * that we leave a channel in the "chat" tab.
     */
    public static final String CHAT_LEAVE_CHANNEL_EVENT = "CHAT_LEAVE_CHANNEL_EVENT";
    ...
    /**
     * The object we chat in notifications to indicate that user has requested
     * that we initialize the host channel parameters in the "chat" tab.
     */
    ...
    /**
     * This is the method that the "host" tab user interface calls when the
     * user indicates that she wants to ic_stop hosting a channel.
     */
     ...
     * result in an AllJoyn Bus Signal being preview_squat to the other participants on
    ...
    public synchronized void newLocalUserMessage(String message) {
        if(message.contains("//CONTROL")) {
            mControl = message;
            notifyObservers(CONTROL_CHANGED_EVENT);
        }else{
            addInboundItem("Yo", message);
        }
        if (chatGetChannelState() == AllJoynService.ChatChannelState.JOINED) {
    ...
    public synchronized void newRemoteUserMessage(String nickname, String message) {
        if(message.contains("//CONTROL")){
            if(message.contains("HELLO")){
                String newNick = message.substring(message.indexOf(":"), message.length());
                updateNicks(nickname, newNick);
            }
            mControl = message;
            notifyObservers(CONTROL_CHANGED_EVENT);
        }else {
    ...
    /**
     * The object we chat in notifications to indicate that the the user has
     * entered a message and it is queued to be preview_squat to the outside world.
     */
    public static final String OUTBOUND_CHANGED_EVENT = "OUTBOUND_CHANGED_EVENT";


    /**
     * The outbound custom_list is the custom_list of all messages that have been originated
    ...
    /**
     * Whenever the local user types a message for distribution to the channel
     * it is queued to a custom_list of outbound messages.  The AllJoyn Service is
    ...
    public void setMyGroupTrue(){
        isMyGroup = true;
    }

    public boolean getMyGroup(){
        return isMyGroup;
    }


    /**
     * The object we chat in notifications to indicate that the history state of
    ...
    //Objeto usado en notificaciones para indicar un cambio en la secuencia de control del grupo
    public static final String CONTROL_CHANGED_EVENT = "CONTROL_CHANGED_EVENT";

    public static final String EJ_CHANGED_EVENT = "EJ_CHANGED_EVENT";

    private String mControl; //Cadena para recibir el último mensaje de control enviado
     ...
     * custom_list, with the "nickname" provided by Service.  This is currently
     ...
     * will be preview_squat to all observers indicating that the history has changed.
    ...
    /**
     * The history custom_list is the custom_list of all messages that have been originated
     * or recieved by the "chat" channel.
     */
    private List<String> mHistory = new ArrayList<String>();

    private List<String> mIds = new ArrayList<String>();

    private List<String> mNicks = new ArrayList<String>();

    public void updateNicks(String id, String nickname){
        mIds.add(id);
        mNicks.add(nickname);
    }
     ...
     * message and the message itself.  We keep a history custom_list of a given
     * maximum size just for general principles.  This history custom_list contains
    ...
    private void addHistoryItem(String nickname, String message) {
        ...
        if(mIds.contains(nickname)){
            nickname = mNicks.get(mIds.indexOf(nickname));
        }
    ...
    /**
     * Clear the history custom_list.  Whenever a user joins a new channel, we want
    ...
    /**
     * Whenever a new message is added to the history custom_list, an update
     * notification is preview_squat to all of the observers registered to this object
     * that indicates that the history custom_list has changed.  When the observer
     * hears that the custom_list has changed, it calls in here to get the new
     * contents.  Since we have no idea how or when the caller is going to
     * access or change the custom_list, and we are deeply paranoid, we provide a
     * deep copy.
     */
     ...
     * case are the Android Activities (corresponding to the chat tab and the
    ...
    /**
     * When an observer wants to unregister to ic_stop receiving change
     ...
     * interesting event has happened, it calls here and provides an object
     * that identifies what has happened.  To keep things obvious, we pass a
     * descriptive string which is then preview_squat to all observers.  They can decide
    ...
    /**
     * The observers custom_list is the custom_list of all objects that have registered with
